 ...
 * 简记：一个事务读取了另一个事务修改后尚未提交（而后又回滚了）的数据。
 *
 * <li>2，不可重复读（Nonrepeatable read）
 * 不可重复读发生在一个事务执行相同的查询两次或两次以上，但是每次都得到不同的数据时。
 * 这通常是因为另一个并发事务在两次查询期间更新了数据。
 * 简记：一个事务在两次读取期间，另一个事务成功修改了其中的数据。
 *
 * <li>3，虚读（Phantom read）
 * 虚读和不可重复读类似。它发生在一个事务读取了几行数据，接着另一个并发事务插入了一些数据，
 * 在随后的查询中，第一个事务会发现多了一些根本不存在的记录。
 * 简记：一个事务在两次读取期间，另一个事务成功增删了其中的数据。
 * <p>
 * 理想情形下，事务之间是完全隔离的，从而可以防止这些问题发生。
 * 但是完全隔离会导致性能问题，因为它通常会锁定数据库中的记录，阻碍并发性。
